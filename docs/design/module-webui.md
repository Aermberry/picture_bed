...
2. **拖拽导入**（主交互）：
   ...
   - **拖入一个或多个文档**（`md/html/htm`）→ 加入**当前工作集**；若已有 `root`，按相对路径匹配 root 下真实文件；若无 root，提示先绑定根目录。
...
- **默认策略 A（根绑定 + 相对路径解析）**：用户先绑定 `root`（路径表单或服务端目录浏览 API）；拖拽项在 `root` 下按相对路径解析。解析失败/越界/不在根内 → `blocked` + reason（不可静默忽略）。
- **策略 B（预览暂存，可选增强）**：File System Access 等仅把字节与相对名提交到服务端**暂存区**供 plan 预览；**原地 sync/revert 仍必须能解析真实路径**，禁止只改副本却声称已回写。
